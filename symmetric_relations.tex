...
\comment{[To-Do] Need to define something like $L(r, d) = \max_{i \in [d]} \sqrt{\lambda_i} \mathtt{LipschitzConstant}(\psi_i)$ as maximum lipschitz constant of first $d$ eigenfunctions? Needed for universal approx result.}
...
\comment{[To-Do] add dependence on lipschitz constant to $\calO(\cdot)$?}
