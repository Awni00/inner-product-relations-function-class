...
\begin{theorem}\label{thm:asymmetric_inner_prod_approximates_rkbs}
   ...
   \noindent where $\phi, \psi: \calX \to \reals^{d}$ are multi-layer perceptrons. Then, for any $\varepsilon > 0$, there exists multi-layer perceptrons with parameters $\theta$ such that
   \begin{equation*}
...
\begin{proof}
    ...
    By~\parencite{}, any two Hilbert spaces with equal dimension are isometrically isomorphic. Hence, without loss of generality, we can restrict our attention to the feature space $\calW = \ell^2(\bbN)$. The dual space is $\calW^* = \ell^2(\bbN)$. Hence, for feature maps $\Phi, \Phi^*$, the ground truth relation to be approximated is,
    \begin{equation*}
    ...
    \begin{equation*}
        \lim_{n \to \infty} \sup_{x,y \in \calX} \abs{r(x, y) - \sum_{j=1}^{n} (\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j} = 0.
    \end{equation*}

    Thus, let $d$ be such that,
    \begin{equation}\label{eq:thm1_proof_eq1}
        \sup_{x,y \in \calX} \abs{r(x, y) - \sum_{j=1}^{d} (\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j} < \frac{\varepsilon}{2}.
    \end{equation}
    ...
    \begin{equation}\label{eq:thm1_proof_eq2}
        \sup_{x \in \calX} \abs{(\phi_\theta(x))_j - (\Phi(x))_{j}} < \tilde{\varepsilon} \ \text{ and } \ \sup_{x \in \calX} \abs{(\psi_\theta(y))_j - (\sigma(\Phi^*(x)))_{j}} < \tilde{\varepsilon}, \ \forall j \in \set{1, \ldots, d}.
    \end{equation}
    ...
    \begin{align*}
        ...
        &\leq \sup_{x,y\in \calX} \paren{\abs{r(x,y) - \sum_{j=1}^{d} (\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j} + \abs{\sum_{j=1}^{d} (\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j - \iprod{\phi_\theta(x)}{\psi_\theta(y)}}} \\
    \end{align*}

    The first term is less than $\varepsilon / 2$ by~\Cref{eq:thm1_proof_eq1}. Now, we bound the second term uniformly on $x,y \in \calX$,
    \begin{align*}
        &\abs{\sum_{j=1}^{d} (\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j - \iprod{\phi_\theta(x)}{\psi_\theta(y)}} \\
        &\leq \sum_{j=1}^{d} \abs{(\Phi(x))_j \cdot (\sigma(\Phi^*(y)))_j - (\phi_\theta(x))_j(\psi_j(y))_j} \\
        &\leq \sum_{j=1}^{d} \abs{(\phi_\theta(x))_j} \abs{(\sigma(\Phi^*(y)))_j - (\psi_j(y))_j} + \abs{(\psi_\theta(y))_j} \abs{(\Phi(x))_j - (\phi_\theta(x))_j}\\
    \end{align*}
