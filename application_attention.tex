...
\begin{theorem}[Attention can approximate arbitrary information selection functions]
    Let $\sset{\preceq_q}_q$ be a family of relevance preorders that satisfies the query continuity and key continuity assumptions~\Cref{ass:key_cts,ass:query_cts}. Suppose that for all $\varepsilon > 0$, there exists $\eta_\epsilon > 0$ uch that 
